...
(:action have-lunch 
	:parameters ( ?x, ?y )
	:precondition (and 
		(PERSON ?x) (not (had-lunch ?x)) (is-person-at ?x ?y)
		(BUILDING ?y) (HAS-RESTURANT ?y)
	)
	:effect (and 
		(had-lunch ?x)
	)
...
(:action attend-morning-lecture
	:parameters ( ?x, ?y, ?z, ?w )
	:precondition (and 
		(PERSON ?x) (IS-STUDENT ?x) (not (had-lunch ?x))
		(PERSON ?y) (IS-TEACHER ?y)
		(BUILDING ?z)
		(LECTURE ?w) (IS-MORNING ?w)
		(IS-LECTURE-AT ?w ?z) (is-person-at ?x ?z) (is-person-at ?y ?z)
		(teaches-lecture ?y ?w)
		(not (attended-lecture ?x ?w))
	)
	:effect (and 
...
(:action attend-afternoon-lecture
	:parameters ( ?x, ?y, ?z, ?w )
	:precondition (and 
		(PERSON ?x) (IS-STUDENT ?x) (had-lunch ?x)
		(PERSON ?y) (IS-TEACHER ?y)
		(BUILDING ?z)
		(LECTURE ?w) (IS-AFTERNOON ?w)
		(IS-LECTURE-AT ?w ?z) (is-person-at ?x ?z) (is-person-at ?y ?z)
		(teaches-lecture ?y ?w)
		(not (attended-lecture ?x ?w))
	)
